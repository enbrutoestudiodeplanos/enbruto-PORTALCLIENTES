...
if (loginView)   loginView.style.display   = 'none';
if (notAuthView) notAuthView.style.display = 'block';
return;
}
...
if (btn) { btn.disabled = false; btn.innerHTML = '<span>Continuar</span>'; }
}
}
...
const emailInput  = document.getElementById('portalEmail');
if (loginView)   loginView.style.display   = 'block';
if (notAuthView) notAuthView.style.display = 'none';
if (emailInput)  { emailInput.value = ''; emailInput.focus(); }
});
}
...
// Si ya hay sesión local vigente, intentar ir directo al panel
const sid = getSessionId();
if (sid) {
