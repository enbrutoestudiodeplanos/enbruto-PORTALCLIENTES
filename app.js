...
details.innerHTML = `<div class="confirm-row"><span>Estudio</span><strong>${data.estudio}</strong></div> <div class="confirm-row"><span>Obra</span><strong>${data.obra}</strong></div> <div class="confirm-row"><span>Medida</span><strong>${data.medida}</strong></div> <div class="confirm-row"><span>Impresión</span><strong>${data.tipo}</strong></div> <div class="confirm-row"><span>Cantidad</span><strong>${data.cantidad} copia${data.cantidad == 1 ? '' : 's'}</strong></div> ${data.notas ?`<div class="confirm-row"><span>Notas</span><strong>${data.notas}</strong></div>`: ''} <div class="confirm-row"><span>Archivo</span><strong>📄 ${data.fileName}</strong></div>`;
...
// Validar archivo (drag&drop o input clásico)
...
// ── Si viene con ?session= desde la landing, guardar y entrar directo ──────
...
setStoredUser(profile);
// Limpiar la URL y entrar al panel
